...
1. 问题现场：输入任意知乎问题标题，可选填原问题链接，再点“整理这个问题”；也可直接打开三个预置议题。
...
**三个预置议题的来源、作者、发言和圆桌是模拟内容。** 自由议题和“使用知乎实时采集”会调用官方搜索与直答接口，并在页面标记为“真实检索样本”。预置圆桌自由追问返回固定演示回应；实时圆桌的角色、知识边界、发言申请、主持人顺序与总结均由当前真实快照生成。发帖、回复、点赞、会话隔离和补充关联是服务端持久化功能。
...
最终演示、上线检查和故障替代方案见 [docs/DELIVERY.md](docs/DELIVERY.md)。
...
**已于 2026-09-11 使用团队配置完成官方接口端到端实测。** 已验证自由标题多轮检索（默认 4 轮查询变体、按内容 ID 与规范化 URL 跨轮去重、目标 30 条唯一主来源，实测单议题 17—21 条唯一来源）、精选评论整理、分类引用校验、实时角色生成、自主逐轮调度、主持人与追问全链路真实生成。凭证只保存在被 Git 忽略的本地环境文件中，不在仓库、日志或浏览器响应中出现。线上环境仍需在托管平台单独配置密钥后再做一次生产验收。
...
- 三个预置议题与任意问题标题入口均已贯通；尚未打包真实浏览器扩展，也未接入知乎 OAuth。
- 采集任务已持久化，支持立即创建、独立查进度、去重和刷新恢复；执行仍由独立 HTTP 请求承载，持久队列尚未接入。
- 真实搜索、分类、角色生成与自主圆桌已完成本地端到端实测；官方接口只能提供相关搜索样本，不能保证某问题的全部回答与全部评论。
